<%@ taglib uri="/WEB-INF/struts-logic.tld" prefix="logic" %>
<%@ taglib uri="/WEB-INF/struts-bean.tld" prefix="bean" %>
<%@ taglib uri="/WEB-INF/struts-html.tld" prefix="html" %>
<%@ taglib uri="/WEB-INF/struts-tiles.tld" prefix="tiles" %>
<%@ page language="java" contentType="text/html;charset=ISO-8859-1"%>
<%
	String type = (String) request.getParameter("type");		
%>
<html> 
<head> 
<title><bean:message key="recherche"/></title>
<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/layout/hcontacts_styles.css">
<link rel="shortcut icon" href="<%=request.getContextPath()%>/img/favicon.ico" type="image/x-icon">	
<script language="JavaScript" src="<%=request.getContextPath()%>/layout/hcontacts_util.js?v4.2"></script>	

<script type="text/javascript"> 
	function showHide() 
	{	
		
	
		var url = window.location.href;
		var type = "<%=type%>";
		var phrase = "";
		if(type == "assures"){
			urlToGo = url.replace("_objets_preload.jsp", "_assures_resultats.jsp");			
		}
		else if( type == "entreprises"){
			urlToGo = url.replace("_objets_preload.jsp", "_entreprises_resultats.jsp");
			
		}
		else{
			urlToGo = url.replace("_objets_preload.jsp", "_appelants_resultats.jsp");
			
		}		
		window.location.href = urlToGo; 		
	

		lancer();
		
	} 
</script> 

</head> 

<body onload="Javascript:showHide();"> 


<div style="padding-top:60px" align="center">
	<table id="att" border="0">
		<tr><td align="center"><div id="pouet"></div></td></tr>
		<tr><td align="center">&nbsp;</td></tr>		
		<tr><td align="center"><label class="noir12">Recherche en cours. Veuillez patienter...</label></td></tr>
	</table>
</div>	



</body> 
</html> 
